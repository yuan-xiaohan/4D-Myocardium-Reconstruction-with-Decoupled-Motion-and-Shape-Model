...
class SurfacePointCloud:
    def __init__(self, mesh, points, normals=None, scans=None, test_sampling=False):
        self.mesh = mesh
        self.points = points
        ...
        if test_sampling:
            self.num_samp_near_surf_ratio = 45 / 50
            self.variance = 0.05
            self.second_variance = self.variance / 100
            self.num_sample = 250000
        else:
            self.num_samp_near_surf_ratio = 47 / 50
            self.variance = 0.005
            self.second_variance = self.variance / 10
            self.num_sample = 500000
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, sampling_type="sphere", use_scans=True,
                                sign_method='normal', normal_sample_count=11, min_size=0, return_gradients=False):
        query_points = []
        # sample on the surface vertices. The initial code is to sample from triangle face.
        surface_sample_count = int(number_of_points * self.num_samp_near_surf_ratio) // 2
        surface_points = self.get_random_surface_points(surface_sample_count, use_scans=use_scans)
        query_points.append(
            surface_points + np.random.normal(scale=np.sqrt(self.variance), size=(surface_sample_count, 3)))
        query_points.append(
            surface_points + np.random.normal(scale=np.sqrt(self.second_variance), size=(surface_sample_count, 3)))
...
def create_from_scans(mesh, bounding_radius=1, scan_count=100, scan_resolution=400, calculate_normals=True, test_sampling=False):
    scans = []
    ...
    return SurfacePointCloud(mesh,
                             ...
                             scans=scans, test_sampling=test_sampling
                             )


def sample_from_mesh(mesh, sample_point_count=10000000, calculate_normals=True, test_sampling=False):
    ...
    return SurfacePointCloud(mesh,
                             ...
                             scans=None, test_sampling=test_sampling
                             )
